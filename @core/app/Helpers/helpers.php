...
function licnese_cheker()
{
	...
	$result = array('license_status'=>'verified','message'=>'your licensed has been activated.');
...
function get_attachment_image_url_by_id($id, $size = null)
{
...
function get_jobs_category_by_id($id, $type = '')
{
    ...
    $jobs = \App\Jobs::find($id);
    $blog_cat = \App\JobsCategory::find($jobs->category_id);
    if (!empty($blog_cat)) {
...
function render_menu_by_id($id)
{
    ...
    foreach ($menu_content as $menu_item) {
        ...
        if ($menu_item->ptype == 'static') {
            ...
            } elseif ($menu_item->ptype == 'gig') {
                $menu_title = '';
                $menu_slug = '';
                $page_details = \App\Gig::find($menu_item->pid);
                if (!empty($page_details)) {
                    $menu_title = $page_details->title;
                    $menu_slug = route('frontend.gigs.single', $page_details->slug);
                    $li_class .= (request()->is(route('frontend.gigs.single', $page_details->slug))) ? ' current-menu-item ' : '';
                }
        ...
        if (!empty($menu_slug) && !empty($menu_title)) {//start condition
            ...
            //append login/register/logout menu for account dropdown
            if ($menu_item->ptype == 'static') {
                if (preg_match('/account/', $menu_item->pname)) {
                    $output .= '<ul class="sub-menu">';
                    if (auth()->guard('web')->check()) {
                        $output .= '<li><a href="' . route('user.home') . '">' . __('Dashboard') . '</a></li>';
                        $output .= '<li> <a href="' . route('user.logout') . '"
                                   onclick="event.preventDefault();
                                   document.getElementById(\'logout-menu-form\').submit();">
                                    ' . __('Logout') . '
                                </a>
                                <form id="logout-menu-form" action="' . route('user.logout') . '" method="POST" style="display: none;">
                                    <input type="hidden" name="_token" value="' . csrf_token() . '">
                                </form></li>';
                    } else {
                        $output .= '<li><a href="' . route('user.login') . '">' . __('Login') . '</a></li>';
                        $output .= '<li><a href="' . route('user.register') . '">' . __('Register') . '</a></li>';
                    }
                    $output .= '</ul>';
                };
            }
                ...
                if ($menu_item->ptype == 'donation_mega_menu') {
                    foreach ($mega_menu_items as $post) {
                        ...
                        if (!empty($post->raised)) {
                            $output .= site_currency_symbol() . $post->raised;
                        } else {
                            $output .= site_currency_symbol() . '0';
                        }
                        $output .= '</h4>';
                        $output .= '<h4 class="raised">' . get_static_option('donation_goal_' . $user_select_lang_slug . '_text') . '' . site_currency_symbol() . $post->amount . '</h4>';
                        $output .= '</div>';
                    ...
                    foreach ($mega_menu_items as $cat => $posts) {
                        ...
                        foreach ($posts as $post) {
                            ...
                            $output .= '<div class="content">';
                            $output .= '<a href="' . $mega_menu_item_slug . '"><h4 class="title">' . $post->title . '</h4></a>';
                            $output .= '<div class="price-wrap"><span class="price">' . site_currency_symbol() . $post->sale_price . '</span>';
                            if (!empty($post->regular_price)) {
                                $output .= '<del class="del-price">' . site_currency_symbol() . $post->regular_price . '</del>';
                            }
                            $output .= '</div></div>';
                            $output .= '</li>';
                        }
    ...
    foreach ($menu_children as $menu_item) {
        ...
        if ($menu_item->ptype == 'static') {
            ...
            if ($menu_item->ptype == 'service') {
                ...
                $page_details = \App\Events::find($menu_item->pid);
                $menu_title = !empty($page_details) ? $page_details->title : '';
                $menu_slug = !empty($page_details) ? route('frontend.events.single', $page_details->slug) : '';
                $li_class .= !empty($page_details) && (request()->is(route('frontend.events.single', $page_details->slug))) ? ' current-menu-item ' : '';
                ...
                $page_details = \App\Jobs::find($menu_item->pid);
                $menu_title = !empty($page_details) ? $page_details->title : '';
                $menu_slug = !empty($page_details) ? route('frontend.jobs.single', $page_details->slug) : '';
                $li_class .= !empty($page_details) && (request()->is(route('frontend.jobs.single', $page_details->slug))) ? ' current-menu-item ' : '';

            } elseif ($menu_item->ptype == 'knowledgebase') {

                $page_details = \App\Knowledgebase::find($menu_item->pid);
                $menu_title = !empty($page_details) ? $page_details->title : '';
                $menu_slug = !empty($page_details) ? route('frontend.knowledgebase.single', $page_details->slug) : '';
                $li_class .= !empty($page_details) && (request()->is(route('frontend.knowledgebase.single', $page_details->slug))) ? ' current-menu-item ' : '';

            } elseif ($menu_item->ptype == 'product') {

                $page_details = \App\Products::find($menu_item->pid);
                $menu_title = !empty($page_details) ? $page_details->title : '';
                $menu_slug = !empty($page_details) ? route('frontend.products.single', $page_details->slug) : '';
                $li_class .= !empty($page_details) && (request()->is(route('frontend.products.single', $page_details->slug))) ? ' current-menu-item ' : '';

            } elseif ($menu_item->ptype == 'donation') {

                $page_details = \App\Donation::find($menu_item->pid);
                $menu_title = !empty($page_details) ? $page_details->title : '';
                $menu_slug = !empty($page_details) ? route('frontend.donations.single', $page_details->slug) : '';
                $li_class .= !empty($page_details) && (request()->is(route('frontend.donations.single', $page_details->slug))) ? ' current-menu-item ' : '';

            } elseif ($menu_item->ptype == 'gig') {

                $page_details = \App\Gig::find($menu_item->pid);
                $menu_title = !empty($page_details) ? $page_details->title : '';
                $menu_slug = !empty($page_details) ? route('frontend.gigs.single', $page_details->slug) : '';
                $li_class .= !empty($page_details) && (request()->is(route('frontend.gigs.single', $page_details->slug))) ? ' current-menu-item ' : '';
...
function render_draggable_menu_by_id($id)
{
    ...
    foreach ($menu_data as $menu):
        ...
        if ($menu->ptype == 'static') {
        ...
        } elseif ($menu->ptype == 'service' || $menu->ptype == 'event' || $menu->ptype == 'work' || $menu->ptype == 'blog' || $menu->ptype == 'job' || $menu->ptype == 'knowledgebase' || $menu->ptype == 'product' || $menu->ptype == 'donation' || $menu->ptype == 'gig') {
            $menu_attr .= ' data-pid="' . $menu->pid . '"';
            ...
            if ($menu->ptype == 'service') {
                ...
                $menu_title = !empty($menu_details) ? $menu_details->title : '';
            } elseif ($menu->ptype == 'gig') {
                $menu_details = \App\Gig::find($menu->pid);
                $menu_title = !empty($menu_details) ? $menu_details->title : '';
            }
...
function render_draggable_menu_children($children, $page_id)
{
    ...
    foreach ($children as $item) {
        ...
        if ($item->ptype == 'static') {
        ...
        } elseif ($item->ptype == 'service' || $item->ptype == 'event' || $item->ptype == 'work' || $item->ptype == 'blog' || $item->ptype == 'job' || $item->ptype == 'knowledgebase' || $item->ptype == 'product' || $item->ptype == 'donation' || $item->ptype == 'gig') {
            $menu_attr .= ' data-pid="' . $item->pid . '"';
            ...
            if ($item->ptype == 'service') {
                ...
                $menu_title = !empty($menu_details) ? $menu_details->title : '';
            } elseif ($item->ptype == 'gig') {
                $menu_details = \App\Gig::find($item->pid);
                $menu_title = !empty($menu_details) ? $menu_details->title : '';
            }
        }
...
function render_mega_menu_item_select_markup($type = '', $menu_id = null)
{
    $output = '';
    $default_lang = get_default_language();
    $menu_details = !empty($menu_id) ? \App\Menu::find($menu_id) : '';
...
function get_job_category_item_count($id)
{
    $count = \App\Jobs::where(['status' => 'publish', 'lang' => get_user_lang(), 'category_id' => $id])->count();
    return !empty($count) ? $count : '0';
}

function get_knowledgebase_topic($id)
{
    $return_val = __("uncategorized");
    $category = \App\KnowledgebaseTopic::find($id);

    if (!empty($category)) {
        $return_val = $category->title;
    }
...
function get_payable_amount_in_usd($pay_am, $output = 'USD')
{
    $site_currency = get_static_option('site_global_currency');

    if ($site_currency == 'EUR') {
        $amount = $pay_am / get_static_option('site_usd_to_eur_exchange_rate');
        if ($output == 'EUR') {
            return number_format($amount, 2);
        }
    } elseif ($site_currency == 'INR') {
        $amount = $pay_am / get_static_option('site_usd_to_inr_exchange_rate');
        if ($output == 'INR') {
            return number_format($amount, 2);
        }
    } elseif ($site_currency == 'IDR') {
        $amount = $pay_am / get_static_option('site_usd_to_idr_exchange_rate');
        if ($output == 'IDR') {
            return number_format($amount, 2);
        }
    } elseif ($site_currency == 'AUD') {
        $amount = $pay_am / get_static_option('site_usd_to_aud_exchange_rate');
        if ($output == 'AUD') {
            return number_format($amount, 2);
        }
    } elseif ($site_currency == 'SGD') {
        $amount = $pay_am / get_static_option('site_usd_to_sgd_exchange_rate');
        if ($output == 'SGD') {
            return number_format($amount, 2);
        }
    } elseif ($site_currency == 'JPY') {
        $amount = $pay_am / get_static_option('site_usd_to_jpy_exchange_rate');
        if ($output == 'JPY') {
            return number_format($amount, 2);
        }
    } elseif ($site_currency == 'GBP') {
        $amount = $pay_am / get_static_option('site_usd_to_gbp_exchange_rate');
        if ($output == 'GBP') {
            return number_format($amount, 2);
        }
    } elseif ($site_currency == 'MYR') {
        $amount = $pay_am / get_static_option('site_usd_to_myr_exchange_rate');
        if ($output == 'MYR') {
            return number_format($amount, 2);
        }
    } elseif ($site_currency == 'PHP') {
        $amount = $pay_am / get_static_option('site_usd_to_php_exchange_rate');
        if ($output == 'PHP') {
            return number_format($amount, 2);
        }
    } elseif ($site_currency == 'THB') {
        $amount = $pay_am / get_static_option('site_usd_to_thb_exchange_rate');
        if ($output == 'THB') {
            return number_format($amount, 2);
        }
    } elseif ($site_currency == 'KRW') {
        $amount = $pay_am / get_static_option('site_usd_to_krw_exchange_rate');
        if ($output == 'KRW') {
            return number_format($amount, 2);
        }
    } elseif ($site_currency == 'NGN') {
        $amount = $pay_am / get_static_option('site_usd_to_ngn_exchange_rate');
        if ($output == 'NGN') {
            return number_format($amount, 2);
        }
    } elseif ($site_currency == 'GHS') {
        $amount = $pay_am / get_static_option('site_usd_to_ghs_exchange_rate');
        if ($output == 'GHS') {
            return number_format($amount, 2);
        }
    } else {
...
{
    $return_val = __("uncategorized");
    $category = \App\ImageGalleryCategory::find($id);
...
function get_gigs_category_by_id($id, $type = '')
{
    $return_val = __('uncategorized');
    $blog_cat = \App\GigsCategory::find($id);
    if (!empty($blog_cat)) {
        $return_val = $blog_cat->name;
        if ($type == 'link') {
            $return_val = '<a href="' . route('frontend.gigs.category', ['id' => $blog_cat->id, 'any' => Str::slug($blog_cat->name)]) . '">' . $blog_cat->name . '</a>';
        }
    }
...
function render_cart_table()
{
    ...
    if (!empty($all_cart_item)) {
        ...
        $output .= '<div class="coupon-wrap"><input type="text" class="form-control" name="coupon_code" placeholder="' . __('Coupon Code') . '"><button class="boxed-btn add_coupon_code_btn">' . __('Submit') . '</button>' . $ajax_preloader . '</div>';
        $output .= '<div class="update-cart-wrap">' . $ajax_preloader . '<button class="boxed-btn update_cart_items_btn">' . __('Update Cart') . '</button></div>';
        $output .= '</div>';
        $output .= '</td></tr>';
        $output .= "\n\t" . '</tfoot>';
...
{
    return get_static_option('product_tax') ? true : false;
}
...
function render_cart_total_table()
{
    ...
    if ($car_total > 0) {
        ...
        $output .= '</div><a href="' . route('frontend.products.checkout') . '" class="boxed-btn">' . __('Process To Checkout') . '</a></div>';
    }
...
{
    $average_ratings = \App\ProductRatings::Where('product_id', $id)->pluck('ratings')->avg();
...
function gig_start_price($id, $symbol = true)
{
    $gig_details = \App\Gig::find($id);
    $plan_price = unserialize($gig_details->plan_price);
    $price = isset($plan_price[0]) ? $plan_price[0] : 0;

    return amount_with_currency_symbol($price);
}
...
function get_username_by_id($id)
{
    ...
    return !empty($user) ? $user->username : __('Unknown');
}

function get_username_by_admin_id($id)
{
    $user = \App\Admin::find($id);
    return !empty($user) ? $user->name : __('Unknown');
}
...
function get_gig_name($gig_id)
{
    $gig_details = \App\Gig::find($gig_id);
    return !empty($gig_details) ? $gig_details->title : 'gig not available';
}
...
function get_package_payment_gateway_by_order_id($id)
{
    $payment_logs = \App\PaymentLogs::where('order_id', $id)->first();
    return !empty($payment_logs) ? str_replace('_', ' ', $payment_logs->package_gateway) : __('unknown');
}
